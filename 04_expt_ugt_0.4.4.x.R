...
exptsList = makeExptsList(num.expt, function(expt) {
  ...
  expt$`reproduction_rate` = 4
  
  # 初始性别比
  expt$`init_sr` = randomIntRange(100, 280) / 100
...
model.path = "/home/lee/NetLogo_models/ugt_0.4.4_x.nlogo"

expt.name = "expt_ugt_0.4.4_x_all_random"
...
# 
distrb = getherParam(rest,"distribute_factor%")
...
df = data.frame(getherVariable(rest,"mid_saving_rate_2")[,i],
                getherVariable(rest,"mean_saving_rate_2")[,i])

df = data.frame(t = seq_along(df),df)
